...
double KgpPlusFluxApproximateRiemannSolver::calculateIntercellFlux(const double &rho_L, const double &rho_R, const double &u_L, const double &u_R, const double &v_L, const double &v_R, const double &w_L, const double &w_R, const double &E_L, const double &E_R, const double &P_L, const double &P_R, const double &a_L, const double &a_R, const int &var_type) {
    ...
    double dissipative_weight      = max( 0.0, pow( sin( min( relative_sos_difference, 1.0 )*0.5*pi ), 0.5 ) );
    ...
    if( var_type == 0 ) {
        ...
        F *= v_L + v_R - dissipative_weight*S*( U_R - U_L );
    } else if ( var_type == 3 ) {
        U_L *= w_L;
        U_R *= w_R;
        S = abs( ( w_L - w_R )/( U_L - U_R + epsilon ) );
        F *= w_L + w_R - dissipative_weight*S*( U_R - U_L );
    } else if ( var_type == 4 ) {
        U_L *= E_L;
        U_R *= E_R;
        S = abs( ( E_L + P_L/rho_L - E_R - P_R/rho_R )/( U_L - U_R + epsilon ) ); 
        F *= E_L + P_L/rho_L + E_R + P_R/rho_R - dissipative_weight*S*( U_R - U_L );
    }
